...
test('generateHandlerFunction simulating successful response', t => {
  try {
    ...
    const createOptions = () => require('./other-lambdas-context-options-legacy.json');
...
test('generateHandlerFunction simulating failure (with custom settings)', t => {
  try {
    ...
    const expectedError = new Error('Kaboom');
    const error = appErrors.toAppError(expectedError);
    ...
    const preFailureCallback = function preFailureCallback(error1, errorResponse1, event1, context1) {
      ...
      t.equal(error1.name, error.name, `preFailureCallback error1.name must be ${error.name}`);
      t.equal(error1.message, expectedError.message, `preFailureCallback error1.message must be ${expectedError.message}`);
